...
// 0.4.1  12/16/22  JPF Re-Integrate 0.3.4 - 0.3.8 with this code body...


char ACS_TITLE[] = "ACS REV: 0.4.1";
...
// external functions and var
extern bool GalilEnable;
extern int readGALILError(void);
extern int chkGALILError(void);
extern int GALILHome(int i);
extern int GALILMax(int i);
extern int GALILClear(int i);
extern int GALILGo(int k); 
extern int CypressHome(int i);
extern int CypressMax(int i);
extern int CypressClear(int i);
extern int CypressGo(int i);
extern void CypressStatus(void);
extern bool GalilEnable;
extern void SDSetTimes(uint32_t startTime, uint32_t stopTime);
extern void SDAppendMotionCypress(uint32_t apc, int16_t current);
...
void WriteCurrentData(char * path)
{
  ...
  file.write(i2s_buff.chars, bufptr*4); //was 2
  file.close();
  //delay(1);
}
...
//=======================================================================
//  A|B Encoder Truth Table
//  Encoder A 1100   
//  Encoder B 0110
//  Logic:
//  A Rising B = 0, B Rising A = 1, A Falling B = 1, B Falling A = 0 CW
//  A Rising B = 1, B Falling A = 1, A Falling B = 0, B Rising A = 0 CCW
//=======================================================================

void doStrutEncoderAr()
{
    int EncB;
    //if A before B
    EncB = digitalReadFast(GALIL_B);
    APC += EncB ? -1 : 1; //rev 0.3.7 JPF, was 1 : -1
    //mymenu[PAGE_GALIL].m[APCREG].mvalue.mval.l = APC;
    attachInterrupt(digitalPinToInterrupt(GALIL_A), doStrutEncoderAf, FALLING);
    //Serial.println("AR");
}

void doStrutEncoderBr()
{
    int EncA;
    EncA = digitalReadFast(GALIL_A);
    //if B before A
    APC += EncA ? 1 : -1; //rev 0.3.7 JPF, was -1 : 1
    //mymenu[PAGE_GALIL].m[APCREG].mvalue.mval.l = APC;
    attachInterrupt(digitalPinToInterrupt(GALIL_B), doStrutEncoderBf, FALLING);
    //Serial.println("BR");

}

void doStrutEncoderAf()
{
    int EncB;
    //if A before B
    EncB = digitalReadFast(GALIL_B);
    APC += EncB ? -1 : 1; //rev 0.3.7 JPF, was 1 : -1
    //mymenu[PAGE_GALIL].m[APCREG].mvalue.mval.l = APC;
    attachInterrupt(digitalPinToInterrupt(GALIL_A), doStrutEncoderAr, RISING);
    //Serial.println("AF");

}

void doStrutEncoderBf()
{
    int EncA;
    EncA = digitalReadFast(GALIL_A);
    //if B before A
    APC += EncA ? -1 : 1; //rev 0.3.7 JPF, was 1 : -1
    //mymenu[PAGE_GALIL].m[APCREG].mvalue.mval.l = APC;
    attachInterrupt(digitalPinToInterrupt(GALIL_B), doStrutEncoderBr, RISING);
    //Serial.println("BF");
}
...
void CheckMenu() {
  ...
  if (!(buttonState = digitalRead(buttonSelectPin)))
  {
    ...
    while(!(buttonState = digitalRead(buttonSelectPin))); // fix button selection
    delay(500);
...
void configmenu(void) {
  ...
  char Teststr[40];
  // TOP LEVEL
  strcpy(mymenu[PAGE_TOP].title, ACS_TITLE);
...
void HighFrequency() { 
    ...
    galil_err = chkGALILError();  // reset the GALIL input
    ...
    */
    ...
    if (GalilEnable) {
      //Serial.println("GalilEnalbed ");
      //Serial.printf("ge = %d, im = %d, GSF = %d", galil_err, inmotion, Galil_SlowFlag);
      //Serial.println("");
      //if (galil_err == false && inmotion == true && Galil_SlowFlag == false) {
      if (galil_err == false && inmotion == true && LastRecord == false) {
        
         //deltaapc = NEWAPC - APC;
         //Serial.printf("delta: %d",deltaapc);
         //Serial.println("");
        ...
        // Look for how fast to send pulses
        
        if (servorate == 0 && inmotion == true  && galil_err == false)
        {
          //Serial.println("HF: pulse");
          if (direction == CCW) { //rev 0.3.7 JPF, was CW
    ...
    //Do the scheduled rate group
    if (brmsTask & brmsRG1Mask) { /* Rate group 1 200us, or 5.0KHz  Equal to a 5KHz update rate. */
        
        if (GalilEnable) {
          galil_err = chkGALILError();  // reset the GALIL input
        }
        
    } else if (brmsTask & brmsRG2Mask) { /* Rate group 2 is run every 400us, or 2.5KHz*/
             
        //if ((recording && inmotion) || (recording_oversample_time > 0))
        //  SDAppendMotionGalil(APC, rawcurrent);
        
    } else if (brmsTask & brmsRG3Mask) { /* Rate group 3 is run every 800us, or 1.25KHz*/
             
    } else if (brmsTask & brmsRG4Mask) { /* Rate group 4 is run every 1.6ms, or 625Hz*/

    } else if (brmsTask & brmsRG5Mask) { /* Rate group 5 is run every 3.2ms, or 312Hz*/
        //if (GalilEnable) {
        //  GALIL_Slow();
        //}
      ...
      if(recording==true  && inmotion == true){
        ...
        i2s_buff.lwords[bufptr++] = (long)abs(deltaapc);
        i2s_buff.lwords[bufptr++] = (long)rawcurrent;
        ACS_Status = LastRecord<<2 +servorate<<1 + galil_err; // collect the status flags
        i2s_buff.lwords[bufptr++] = (long)ACS_Status; 
        etime = (long)(micros() - GalilStartTime);
        i2s_buff.lwords[bufptr++] = (long)etime;
        ACS_Status = 0; // clear for next time
        ...
        if(LastRecord==true && inmotion == true){
          if(cycles_to_comp > 0)
          {
            cycles_to_comp--;
          }
          else
          {
            ...
            mymenu[PAGE_GALIL].m[ERR].mvalue.mval.l = deltaapc;
            mymenu[PAGE_GALIL].m[APCREG].mvalue.mval.l = APC;
            DrawMenus(pagenum);  
          }
        }
         
      } else if (brmsTask & brmsRG6Mask) { /* Rate group 6 is run every 6.4ms, or 156Hz*/
        /*if(recording==true  && inmotion == true){
        //Serial.println("Write to SD");
        rawcurrent = ina219g.getCurrent_raw();
        i2s_buff.lwords[bufptr++] = (long)APC; 
        i2s_buff.lwords[bufptr++] = (long)abs(deltaapc);
        i2s_buff.lwords[bufptr++] = (long)rawcurrent;
        ACS_Status = servorate<<1 + galil_err; // collect the status flags
        i2s_buff.lwords[bufptr++] = (long)ACS_Status; 
        etime = (long)(micros() - GalilStartTime);
        i2s_buff.lwords[bufptr++] = (long)etime;
        ACS_Status = 0; // clear for next time
        ...
        */
        if (GalilEnable && servorate == 1 && inmotion == true)
        {
        deltaapc = NEWAPC - APC;  // calculate the difference in position
        ...
        if(deltaapc >= 0) 
          ...
          //Serial.println("Galil Slow...");
            ...
            if(abs(deltaapc)<=2 && LastRecord==false){
              GalilEndTime = micros();
              ...
              inmotion = true;
              //servorate = 2;  // done...
              //Serial.println("Move Done! - set comp = 200");
        ...
        brmsTask = 0; // complimentary rate group, start over
    }
}
...
void setup() {
  ...
  Serial.print(">>> initializing SD Card"); Serial.println(x, HEX);
  if (!SD.begin(chipSelect)) 
  {
    Serial.print(">>> SD initialization failed!"); Serial.println(x, HEX);
    filestatus = 1;
  }
  else
  {
    Serial.print(">>> initialization done."); Serial.println(x, HEX);
    // If the data.txt file doesn't exist
    // Create a file on the SD card and write the data labels
    file = SD.open(filename);
    if(!file) {
      Serial.print(">>> File doens't exist"); Serial.println(x, HEX);
      Serial.print(">>> Creating file..."); Serial.println(x, HEX);
      //writeFile(filename, "current\r\n"); 
      filestatus = 1;
    }
    else {
      Serial.print(">>> File already exists"); Serial.println(x, HEX);  
      filestatus = 1;
    }
    file.close();
  }
...
void loop(void) {
  ...
  if (write_file || CypressWriteFile) {
      ...
      inmotion = false;
      LastRecord = false;
      recording = true;   /// JPF reset for new recording
      interrupts();
